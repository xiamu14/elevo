...
export class StateFileParser {
  ...
  async parseStateFile(filePath: string): Promise<StateFileInfo | null> {
    try {
      ...
      const machineInstance = await this.extractMachine(compiledContent, filePath);
  ...
  private extractMachineName(content: string, fallbackName: string): string {
    ...
    const machineNameMatch = content.match(/createMachine\\s*\\(\\s*[\"']([^\"']+)[\"']/);
    if (machineNameMatch) {
      return machineNameMatch[1];
    }
    
    // Try to find exported machine variable name
    const exportMatch = content.match(/export\\s+(?:const|let|var)\\s+(\\w+)\\s*=/);
    if (exportMatch) {
  ...
  private async compileTypeScript(content: string, filePath: string): Promise<string> {
  ...
  private async extractMachine(content: string, filePath: string): Promise<any> {
    try {
      ...
      let processedContent = content
        .replace(/import\\s+{([^}]+)}\\s+from\\s+[\"']elevo[\"']/g, 'import { $1 } from "elevo"')
        .replace(/import\\s+{([^}]+)}\\s+from\\s+[\"']elevo-react[\"']/g, 'import { $1 } from "elevo-react"');
      
      // Add export to make it importable
      if (!content.includes('export')) {
        // Try to find the machine variable and export it
        const machineMatch = content.match(/(const|let|var)\\s+(\\w+)\\s*=\\s*createMachine/);
        if (machineMatch) {
      ...
      try {
        ...
        const machine = module.default || Object.values(module).find((exp: any) => 
          exp && typeof exp === 'object' && typeof exp.toXStateJSON === 'function'
        );
